...
func NewServerConfiguration(conf *config.Couper, log *logrus.Entry, memStore *cache.MemoryStore,
	...
	for _, srvConf := range conf.Servers {
		...
		if srvConf.Spa != nil {
			...
			spaHandler = configureProtectedHandler(accessControls, confCtx,
		...
		if srvConf.Files != nil {
			...
			protectedFileHandler := configureProtectedHandler(accessControls, confCtx,
				config.NewAccessControl(srvConf.AccessControl, srvConf.DisableAccessControl),
		...
		for endpointConf, parentAPI := range endpointsMap {
			...
			endpointHandlers[endpointConf] = configureProtectedHandler(accessControls, confCtx, accessControl,
...
func configureProtectedHandler(m ACDefinitions, ctx *hcl.EvalContext, parentAC, handlerAC config.AccessControl,
	opts *protectedOptions, log *logrus.Entry) http.Handler {
	var list ac.List
	...
	if len(list) > 0 {
		return handler.NewAccessControl(opts.handler, list)
	}
	return opts.handler
}
